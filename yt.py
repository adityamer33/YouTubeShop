...
def youtube_session(email, password):
    try:
        ...
        status = authenticator.ServiceLogin('youtube',
                                            'https://www.youtube.com/signin?app=desktop&next=%2F&hl=en&action_handle_signin=true')
        counters('login-t')
        return status
...
for identifier in cli.read_action_file(action_path):
    for credentials in cli.read_accounts_file(accounts_path):
